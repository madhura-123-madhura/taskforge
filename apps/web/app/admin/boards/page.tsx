...
import { useCreateTaskMutation, useDeleteTaskDetailsMutation, useFetchTaskQuery, useGetEmployeesQuery, useUpdateTaskDetailsMutation, useUpdateTaskMutation } from '@/redux/apis/admin.api'
...
const TaskCard = ({ t }: { t: Task }) => {
    ...
    const [updateDetail, { isLoading: updateLoading }] = useUpdateTaskDetailsMutation()
    const [deleteDetail, { isLoading: deleteLoading }] = useDeleteTaskDetailsMutation()
    ...
    const handleFormSubmit = async (taskData: UPDATE_TASK_DETAILS_REQUEST) => {
        try {
            ...
            if (taskData.title) fd.append("title", taskData.title)
            ...
            await updateDetail({ id: t.id, fd }).unwrap()
    ...
    const handleDelete = async () => {
        try {
            await deleteDetail({ id: t.id }).unwrap()
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)}>
                    ...
                    <FieldGroup>
                        <Field>
                            ...
                            <Input {...register("title")} id="task" name="task" />
                        </Field>
                        <Field>
